...
renderCatalog (отрисовка катлога)
...
Класс GeneralUI
Реализация показа суммы товара в модальном окне товара
за счет тернарного оператора будет реализовываться либо показ суммы или 'Бесценно'

Класс AllModal
Поле modal (общий контейнер)
Поле modals (все модальные окна с классом modal)
Поле cardModal (карточка)
Поле basketModal (работа с модальным окном корзины)
Поле orderModal (модальное окно заказа)
Поле contactsModal (модальное окно контактов)
Поле successModal (модальное окно оформление заказа)

Конструктор 
-присваивает к тегам переменные
- реализуются функции 
setModal() сборщик контента в модальном окне 
modalOpen() открыие модального окна 
